    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Text("Cuentas Vinculadas").font(.system(size: 18, weight: .semibold)).foregroundColor(.primary)
                Spacer()
            }
            .padding(16).background(Color.white).shadow(color: Color.black.opacity(0.05), radius: 4, y: 2)
            ScrollView { VStack(alignment: .leading, spacing: 16) { Text("Módulo de Cuentas Vinculadas").font(.system(size: 20, weight: .bold)).padding(16); Spacer() } }
            Spacer()
        }
